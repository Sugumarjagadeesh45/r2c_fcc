...
//   const getProfileImage = (user, id) => {
//     if (!user) return null;
    
//     if (id && imageErrors[id]) return null;
...
//     const profileImage = getProfileImage(userObj, userId);
...
const FriendsScreen = () => {
  ...
  const getAvatarUrl = (userObj) => {
    ...
    // Check for avatar field first as per backend notes
    let uri = userObj.avatar;
    
    // Also check for other possible fields for backward compatibility
    if (!uri) {
      uri = userObj.profilePicture || 
            userObj.photoURL || 
            userObj.avatarUrl ||
            (userObj.user && (userObj.user.avatar || userObj.user.profilePicture));
    }
    
    // Return null if no avatar found (as per backend notes)
    if (!uri) return null;
    ...
    try {
      ...
      try {
        ...
        if (friendsResponse.ok) {
          ...
          // Log all friends data after setting the state
          // Use a timeout to ensure state is updated before logging
          setTimeout(() => {
            logAllFriendsData();
          }, 100);
        } else {
  ...
  const renderFriend = ({ item, index }) => {
    ...
    const userName = item.name || item.username || 'Unknown Friend';
    const userStatus = item.status || item.bio || 'Friend';
    const userInitials = getUserInitials(userName);
    const avatarColor = getAvatarColor(userName);
    ...
    console.log(`Rendering friend ${userName} with avatar URL: ${avatarUrl}`);
        ...
        <TouchableOpacity onPress={() => handleViewProfile(item)} style={styles.avatarContainer}>
          {avatarUrl ? (
            ...
            <Image 
              ...
              style={styles.avatarImage}
              onError={() => console.log('Failed to load avatar for', userName)}
            />
          ) : (
  ...
  const renderSuggestion = ({ item, index }) => {
    ...
    // Get avatar URL using the helper function
    const avatarUrl = getAvatarUrl(userObj);
